...
土壌湿度センサーから土壌水分量のアナログ値を6時間ごとに取得し、設定した閾値を下回った場合に給水ポンプを3秒間動作させます。<br>

また、2つのボタンによる手動操作にも対応しており、左ボタンで現在の土壌湿度ステータス（SUPER DRY / DRY / WET）をモニタに表示し、右ボタンで1秒間だけ給水させることができます。

センサー値の取得、乾燥判定、ポンプ制御、モニタ表示を、組み込みソフトウェアとして実装しています。
...
* **課題:** 
  当初、Raspberry Pi Pico の GPIO ピン（3.3V / 電流制限あり）からリレーモジュールへ直接接続した際、電力が不足しリレーを正常にスイッチング動作させることができませんでした。
* **解決策:** 
  マイコンのピン出力を直接給電に使わず、NPNトランジスタ（S8050）とベース抵抗（1kΩ）を組み合わせたスイッチング回路を配置。Pico からの信号でトランジスタを制御し、リレーへ安定した電流を供給することで確実な動作を実現しました。

### 2. 電源系統の分離によるノイズ・電圧降下対策
* 水中ポンプ動作時のノイズや急速な電圧降下が Pico（制御側）に及ぼす影響を防ぐため、ロジック電源（18650充電モジュール）と駆動用電源（単4電池×4）の系統を物理的に分離しました。

## 🚀 今後の展望（ESP32 への拡張計画）

現在は Pico によるスタンドアロン環境で動作していますが、今後は Wi-Fi / Bluetooth 通信モジュールを搭載した **ESP32** への移設・アップデートを計画しています。

* **IoT化とデータの可視化:** 
  土壌湿度の推移データをクラウド（Webサーバー/ダッシュボード）に蓄積し、グラフで可視化する。
* **スマートフォン連携:** 
  「乾燥状態が一定時間続いた場合」や「給水完了時」に、LINE や Discord などの API を経由してスマホへ通知を送る機能を実装予定。
