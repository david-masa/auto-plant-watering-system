...
* **解決策:** 
  Raspberry Pi Picoから直接リレーに接続するのではなく、NPNトランジスタを中間に配置し、リレーの動作を確実なものにしました。
...
現在はRaspberry Pi Picoによる環境で動作していますが、今後は Wi-Fi / Bluetooth 通信モジュールを搭載した **ESP32** への載せ替えを計画しています。

* **IoT化** 
  土壌湿度のデータをクラウドサーバーに保存し、別のマイコンから遠隔で閲覧できるようにする。
* **スマートフォン連携:** 
